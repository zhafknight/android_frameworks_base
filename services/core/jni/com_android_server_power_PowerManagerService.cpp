...
bool getLineagePowerHal() {
    ...
    return gLineagePowerHal != nullptr;
}
...
static jbyte nativeGetSupportedProfilesCount(JNIEnv *env, jclass clazz) {
    ::vendor::lineage::power::V1_0::SupportedProfiles count =
            ::vendor::lineage::power::V1_0::SupportedProfiles::UNSUPPORTED;
    std::lock_guard<std::mutex> lock(gPowerHalMutex);
...
static void nativeSetProfile(JNIEnv *env, jclass clazz, jbyte profile) {
    std::lock_guard<std::mutex> lock(gPowerHalMutex);
    if (getLineagePowerHal()) {
        Return<void> ret = gLineagePowerHal->setProfile(
                static_cast<::vendor::lineage::power::V1_0::Profile>(profile));
...
static void nativeBoost(JNIEnv *env, jclass clazz, jint duration) {
    std::lock_guard<std::mutex> lock(gPowerHalMutex);
    if (getLineagePowerHal()) {
        Return<void> ret = gLineagePowerHal->boost(static_cast<uint32_t>(duration));
